...
import { isFunction, isString, isNumber, isArray, noop } from '@wisewe-framework-v4/utils/tools'
...
function genFilterKey(event: any, keyFilter: any) {
  ...
  for (const key of genArr) {
    ...
    if (
      ...
      (aliasKeyCode && isArray(aliasKeyCode) ? aliasKeyCode.includes(event.keyCode) : aliasKeyCode === event.keyCode) ||
...
export function useKeyPress(keyFilter: KeyFilter, eventHandler: EventHandler = noop, option: EventOption = {}) {
...
export function getTargetElement(target?: Target, defaultElement?: TargetElement): TargetElement | undefined | null {
  if (!target) {
