...
const getOutFile = (name, dir = 'lib') => {
  const compName = name.split('@wisewe-framework-v4/')[1]
...
const inputs = getPackagesSync()
  .map(pkg => pkg.name)
  .filter(name =>
    name.includes('@wisewe-framework-v4') &&
    !name.includes('utils'),
  )

export default inputs.map(name => ({
  input: path.resolve(__dirname, `../packages/${name.split('@wisewe-framework-v4/')[1]}/index.ts`),
  output: [{
    format: 'es',
